...
为了方便查阅，本项目内的一些重要产出与数据存储在以下关键位置：

- **PPT 展示文档**：已生成项目汇报所用的幻灯片，保存在根目录的 `Presentation.pptx` 文件中。
- **Paper (学术论文原稿)**：本细粒度情感识别机制的完整原理及 SOTA 性能论证分析，位于 `ERC1/paper/` 目录下（包含 `main.tex` 和 `ERC.pdf`）。
- **数据集文件**：
